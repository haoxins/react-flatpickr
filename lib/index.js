...
class DateTimePicker extends Component {
  ...
  componentWillReceiveProps(props) {
    if (props.options.value) {
      this.flatpickr.setDate(props.options.value)
    }
  }
  ...
  render() {
    ...
    const { onChange, options, defaultValue, ...props } = this.props
    const { id } = this.state
    return (
      <input {...props} id={id} defaultValue={defaultValue || options.value}/>
    )
  }
}
